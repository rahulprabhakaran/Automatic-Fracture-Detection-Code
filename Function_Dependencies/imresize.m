...
order = images.internal.resize.dimensionOrder(params.scale);
...
for k = 1:params.num_dims
    [weights{k}, indices{k}] = images.internal.resize.contributions( ...
        size(A, k), ...
        params.output_size(k), params.scale(k), params.kernel, ...
        params.kernel_width, params.antialiasing);
    
    if ~images.internal.resize.isPureNearestNeighborComputation(weights{k})
...
if allDimNearestNeighbor
    ...
    B = images.internal.resize.resizeAllDimUsingNearestNeighbor(A, indices);
...
params.kernel = @images.internal.resize.cubic;
...
first_param_string_idx = images.internal.resize.findFirstParamString( ...
...
kernels = {@images.internal.resize.box, ...
           @images.internal.resize.triangle, ...
           @images.internal.resize.cubic, ...
           @images.internal.resize.box, ...
           @images.internal.resize.triangle, ...
           @images.internal.resize.cubic, ...
           @images.internal.resize.lanczos2, ...
           @images.internal.resize.lanczos3};
...
param_check_fcns = {@processScaleParam, ...
                    ...
                    @processMethodParam, ...
                    @images.internal.resize.processAntialiasingParam};
...
if isempty(params.scale)
    params.scale = images.internal.resize.deriveScaleFromSize(params);
end

if isempty(params.output_size)
    params.output_size = images.internal.resize.deriveSizeFromScale(params);
end
%---------------------------------------------------------------------
...
if images.internal.resize.isPureNearestNeighborComputation(weights)
    out = images.internal.resize.resizeAlongDimUsingNearestNeighbor(in, ...
        dim, indices);
    return
end
