...
% specify folder where shearlet systems are saved before '\shearletSystem'
outfolder = 'D:\PhD\Automatic_Detection\Probabilistic_Edges\Combined_Shearlet_Systems\';
...
no_Shearlets = 70; % I set 70 because I have a pre-defined set of shearlets
                   % of 70 that have given good results so far
...
% normalized ridge image. If the images are 
...
output_folder = 'D:\PhD\Automatic_Detection\Tiled_Orthomosaics\Broadmere_1_2\P_Ridges\';
...
% Loop runs over each image
 ...
 for i=1:70 % number of shearlet systems, I have only 70 shearlets
     
    tic
     % loading shearlet systems from the shearlet folder
     load(strcat(outfolder,num2str(i),'.mat'));
  ...
  for j=1:15 % number of ridge realizations per shearlet. I choose 15
             % as I only want to run a total of 70*15 = 1050 realizations
    ...
    % to save each ridge to see the effects of each 
    ridges2 = imresize(ridges,[imageIN_Xpixels,imageIN_Ypixels]);
    imwrite(ridges2, strcat([output_folder,'P_Ridges_',num2str(i),'_','MC',num2str(RidgeExtract_Combs(j,1)),'_Off',num2str(RidgeExtract_Combs(j,2)),'.png']));
 ...
 clearvars C_Edges_norm C_Edges imageIN outfilename
 toc
